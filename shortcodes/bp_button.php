...
function bp_shortcode_button( $atts, $content = null ) {
    ...
    $modalTarget      = (!empty($modal_target) ? "data-remodal-target=\"".$modal_target."\"" : '');
    ...
    if ( $circle == 'true' ) {
        ...
        $output = "<a {$id} class=\"x-btn{$class}{$type}{$shape}{$size}{$block}{$float}{$icon_only}\" {$style} href=\"{$href}\" {$modalTarget} {$title} {$target} {$tooltip_attr} {$lightbox_caption} {$lightbox_options}>" . do_shortcode( $content ) . "</a>";
    }

    return $output;
}

add_shortcode( 'bp_button', 'bp_shortcode_button' );