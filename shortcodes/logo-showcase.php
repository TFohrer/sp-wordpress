...
<?php if (isset($smls_settings['logo_layout']) && $smls_settings['logo_layout'] == 'grid') {
  Timber::render('logo-grid.twig', ['entries' => $smls_option['logo']]);
  return;
} ?>

<div class="logo-showcase-slider logo-showcase-slider--<?php echo $random_num; ?>">
    <div class="logo-showcase-slider__container">
        <?php foreach ($smls_option['logo'] as $logo_index => $logo_serialized_detail) {
          if (is_numeric($logo_index)) {
            ...
            <div class="logo-showcase-slider__slide">
                <img src="<?php echo esc_attr($smls_option['logo'][$logo_key]['logo_image_url']); ?>"
                     alt="<?php if (isset($smls_option['logo'][$logo_key]['title'])) {
